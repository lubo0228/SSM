		...
		<div class="tab-pane active" id="tab1">
			...
			<div class="panel panel-info">
				...
				<div class="panel-body">
					...
					<c:forEach items="${prizes }" var="prizes">
						<option value="${prizes.prizeId }">${prizes.prizeName }</option>
					</c:forEach>
					</select>
					<!-- 数量:
				...
				<div class="table-responsive panel-body">
					...
					<div style="padding: 15px;">
						...
						<table class="table table-hover">
							...
							<c:forEach items="${records}" var="records" varStatus="i">
								<tr><td hidden="hidden">
								    <input type="text" class="form-control" value="${i.index}">
									<input type="text" class="form-control" name="recordList[${i.index}].recordsId" value="${records.recordsId}">
									<input type="text" class="form-control" name="recordList[${i.index}].prizeId" value="${records.prizeId}">
								    <input type="text" class="form-control" name="recordList[${i.index}].prizeType" value="${records.prizeType}">
								    </td>
								    <td style="text-align:center;vertical-align:middle;">
								    <input id="prizeLevel" name="recordList[${i.index}].prizeLevel" class="form-control" style="padding-left:9px;" value="${records.prizeLevel}" readonly="readOnly"></td>
								    <td style="text-align:center;vertical-align:middle;">
								    <input type="text" class="form-control" name="recordList[${i.index}].prizeName" value="${records.prizeName}" id="priName" maxlength="7" readonly="readOnly"></td>
								    <td style="text-align:center;vertical-align:middle;">
								    <input type="text" class="form-control" name="recordList[${i.index}].whiteUserNum" value="${records.whiteUserNum}" id="prizeCount" maxlength="8" onchange="checkPrizeCount2(this,${records.prizeId})"></td>
								    <c:if test="${records.prizeType == 1}">
									    <td style="text-align:center;vertical-align:middle;">
									    <select name="recordList[${i.index}].prizeType" class="form-control" id="priType" disabled="disabled"><option value="1">实物</option></select></td>
								    </c:if>
								    <c:if test="${records.prizeType == 2}">
									    <td style="text-align:center;vertical-align:middle;">
									    <select name="recordList[${i.index}].prizeType" class="form-control" id="priType" disabled="disabled"><option value="2">积分</option></select></td>
								    </c:if>
								    <td style="text-align:center;vertical-align:middle;">
								    <input type="text" class="form-control" name="recordList[${i.index}].points" value="${records.points}" id="prizePoint" maxlength="8" readonly="readOnly"></td>
								    <td><a class="btn  btn-default" rel="tooltip" href="javascript:void(0)" onclick="removePrize2(this,${records.recordsId})" title="删除"><i class="glyphicon glyphicon-remove"></i></a></td>
								    </tr>
								</c:forEach>    
							</tbody>
						</table>
...
/*添加奖品*/
var index =parseInt($(".prize-items tr").last().children().first().find("input").val())+1;
console.log(index)
if(undefined == index){
 	index = 0;
}
...
function removePrize2(obj,recordId) {
	arrPIds.push(recordId);
	$(obj).parent().parent().remove();
	$('#deleteRecord').val(arrPIds);
}
/* 白名单奖品数量不能大于奖品总数 */
...
function checkPrizeCount2(obj,prizeId){
	 var reg =/^[1-9]\d*$/;    
     var num =$(obj).val();
     if(!reg.test(num)){
        layer.alert("奖品数必须正整数！");
...
function summitWhiteUser(){
	...
	if(flag){
		$.ajax({
			url:'${ctx}/wheel/user/sumbitEditWhiteUser',
			data :$('#whiteUser').serialize(),
			type : 'post',
			timeout : 0, 
			success : function(result) {
				if(result){
					layer.alert("修改白名单成功",{closeBtn:0,icon:1},function(){window.history.go(-1);layer.close(layer.index);});
				}else{
					layer.alert("修改白名单失败");
				}
			},
...
<!-- <script>
	...
	function addWhiteUser(openId, activityTitle, unicalId) {
		...
		layer.open({
			...
			fixed : false, // 不固定
			maxmin : true,
			content : '../../webpage/modules/game/recordList.jsp'
		});
	}
